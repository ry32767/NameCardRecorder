...
1. **登録** — 「+ 名刺を追加」→ カメラで撮影、または画像を選択。画面には片面だけが出るので、裏はカードを押して裏返してから読み込む。数秒で文字認識が走り、表の画像から拾えた項目がフォームに入る。**候補なので必ず目で確かめて直す。**画像の上に出た行を押せばその行をコピーできる。必要なら「どこで会ったか」やタグを足して保存。
