...
- **撮影して登録** — カメラまたは画像ファイルから名刺を読み込み、ブラウザ内の OCR で氏名・会社・連絡先を抽出。確認フォームで直してから登録する。
...
│   │   ├── ocr/           # 画像前処理・Tesseract 実行・項目抽出（parser）
...
テストは対象と同じ場所に `*.test.ts(x)` で置いている（例: `src/lib/ocr/parser.test.ts`）。
...
1. **登録** — 「+ 名刺を追加」→ カメラで撮影、または画像を選択。数秒で文字認識が走り、氏名・会社・メールなどが埋まったフォームが出る。間違いを直し、必要なら「どこで会ったか」やタグを足して保存。
