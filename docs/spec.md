...
- [x] OCR エンジンを **PaddleOCR（既定）** と **Tesseract.js** から選べ、次に読み取る名刺から切り替わる
- [x] エンジンを選ぶ場所に、**どちらもブラウザ内で完結し名刺画像が OCR サービスへ送られないこと**が明記されている
- [x] 「認識テキストを画像上に表示」を設定で ON / OFF でき、再読み込み後も保たれる
- [x] エンジンを変えても、リポジトリ・トークンの設定は巻き添えで変わらない
- [x] 以前保存した Cloud Vision の API キーは、設定を読んだ時点で端末から消える（Cloud Vision は 2026-09-09 に廃止）
...
- [x] モデル・言語データの取得中は「OCR モデルを読み込んでいます…」と出て、認識中と区別できる
- [x] PaddleOCR のモデルを取れなかったときは理由が出て、**Tesseract.js で読み直す**入口が出る（読めなくても手入力で登録できる）
- [x] OCR 中も画面が固まらない（Web Worker で実行されている）
- [ ] 標準的な日本語名刺の OCR が、**画像 1 枚あたり** PC で 10 秒以内・スマホで 20 秒以内に終わる（2 枚選んだ場合は 1 枚ずつ順に読み取る）
      → **PC は確認済み**（PaddleOCR: 初回 20.0 秒＝モデル 21MB の取得込み / 2 回目 4.1 秒）。**スマホは未確認**
- [ ] JPEG / PNG / HEIC のいずれでも読み込める（HEIC が不可の環境では `この形式の画像は読み込めません` と表示する）
- [x] 画像以外のファイルを選ぶと `画像ファイルを選んでください` と表示され、OCR は始まらない

#### 機能2：確認フォームと登録

抽出結果は必ず人が直せる。**OCR の精度そのものは受け入れ条件にしない**（エンジン側の限界があるため）。判定するのは「印字されている情報が正しくフィールドに振り分けられるか」の機械的な部分に絞る。
...
- [x] ~~OCR プロバイダの切り替え~~ → **PaddleOCR を既定にして実装済み**（2026-09-09）。クラウド OCR / LLM は不採用（画像を外に出さない前提を優先）
...
| プライバシー | 名刺画像・個人情報は private リポジトリの外に出さない。**OCR はどちらのエンジンでもブラウザ内で完結し、画像を外部送信する経路を持たない**（外に出るのはモデル・言語データを取りに行く GET だけ） |
...
| PaddleOCR の戻り値の変換（poly → bbox・読み順の並べ直し・空行の除去・信頼度） | `src/lib/ocr/paddleResult.test.ts` | 自動 |
| エンジンの選択（設定で切り替わる・渡す画像が違う） | `src/lib/ocr/engine.test.ts` | 自動 |
| 設定の読み込みと移行（欠けた項目の既定値・廃止した API キーの削除） | `src/lib/settings.test.ts` | 自動 |
| **OCR の設定変更で一覧のキャッシュを捨てないこと**（捨てるのはリポジトリ・トークンを変えたときだけ） | `src/lib/settings.test.ts` / `src/store/SettingsProvider.test.tsx` | 自動 |
| 重ね表示の ON/OFF（OFF→ON で読み直さない・設定に従う） | `src/pages/NewCard.test.tsx` | 自動 |
| PaddleOCR が読めなかったときの逃げ道（理由の表示・Tesseract での読み直し） | `src/pages/NewCard.test.tsx` | 自動 |
...
**自動テスト 266 / Lint / 型 / ビルド すべて green（CI でも同じ検証ループが通っている）。**
> 268 から減ったのは、Cloud Vision のテスト（11 件）を実装ごと削り、代わりに PaddleOCR の変換
> （6 件）・設定の読み込みと移行（4 件）・キャッシュを捨てる条件（5 件）・重ね表示の ON/OFF と
> 読み直し（3 件）を足したため。
...
| **実際の OCR（架空の名刺を合成して実行）** | ビルド成果物（`npm run preview`）で canvas から架空の名刺画像を作り、PaddleOCR に読み取らせて**フォームへの自動入力まで確認**：氏名・会社名・部署・役職・メール・電話・携帯・FAX・郵便番号・住所・Web サイトの**11 項目すべてが入った**（Tesseract では氏名が取れていなかった）。重ねた行は 9 本。住所・部署に文字の欠けが少し残る |
...
| **OCR の所要時間（PC）** | 同上の実測。PaddleOCR で**初回 20.0 秒**（モデル 21MB の取得を含む）／**2 回目 4.1 秒**（取得後）。2 回目以降は PC 10 秒以内。初回の内訳はほぼダウンロードで、SDK の初期化＋認識自体は約 4.5 秒（モデル取得済みなら 1.5 秒 + 3.1 秒） |
| **スマホで開いたときに重くならないこと** | ビルド成果物の chunk を確認。エントリは **223KB**（PaddleOCR 以前と同じ構成）で、PaddleOCR の 10MB 超の chunk・Worker・モデルは**実際に読み取るまで取りに行かない**（`index.html` はエントリしか参照せず、10MB の chunk は `paddle` chunk からのみ到達可能）|
| **重ね表示の ON/OFF** | ビルド成果物で実測。OFF で重ねた行が 0 本になり、ON に戻すと**読み直さずに**9 本が戻る。設定は localStorage に残る |
...
- **スマホでの PaddleOCR の実行可否と所要時間**。モデル 21MB の取得と WASM 推論が非力な端末で
  どこまで通るかは実機でないと分からない。通らない場合に備えて Tesseract.js に切り替えられるようにしてある
...
- [x] ~~Tesseract の言語データの配信方法~~ → **CDN 参照に決定**（同梱しない）。PaddleOCR のモデルも同じ扱い。
      理由と影響は [architecture.md](architecture.md#モデルと言語データの配信)。
      **未決定のまま残るのは所要時間の実測**：初回はモデル・辞書の DL 時間が乗るため、性能要件（PC 10 秒／スマホ 20 秒）は 2 回目以降を指すものとして扱う。スマホで計測して数値を追記すること
- [x] ~~読み取り結果（座標つき）を名刺と一緒に保存するか~~ → **保存しない**（2026-09-09 に決定）。
      重ね表示は登録画面で内容を確かめるためのもので、詳細画面で開き直したときに要るものではない。
      Issue には引き続き `ocrText` の生テキストだけ残す。**Issue 本文のスキーマは変えない。**
- [ ] **PaddleOCR がスマホで実用になるか。** モデル 21MB の取得と WASM 推論が非力な端末で通るかは実機次第。
      通らない場合、既定を端末で出し分けるか、Tesseract.js への切り替えを案内するだけにするかを決める
