...
- [x] Cloud Vision に拒否されたとき、**何を直せばいいか**が分かる（API 未有効化・リファラー制限・API の制限・課金の別が出る。知らない理由なら Google の原文を添える）
...
**自動テスト 268 / Lint / 型 / ビルド すべて green（CI でも同じ検証ループが通っている）。**
> 231 から増えたのは、Cloud Vision（11 件）・エンジンの選択（4 件）・重ね表示とコピー・
> 表裏の切り替え・画像の選び直し・Vision の失敗理由の翻訳（7 件）のテストを足したため。
