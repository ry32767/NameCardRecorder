...
- [x] 読み取った文字が**行ごとに一覧表示**される（[方針](architecture.md#項目の振り分けはしない方針転換)）
- [x] 行をタップするとその行だけがクリップボードにコピーされ、コピーできたことが分かる
- [x] 日本語が文字単位にばらけて読まれた行（`株 式 会 社 …`）は詰めて表示する。ただし `山田 太郎` の姓名の区切りや `TEL 03-…` の空白は残す
- [x] 表と裏を読み取った場合、どちらの面の行かが分かるように分けて表示される
- [x] クリップボードが使えない環境では、その旨を出して手で選択できる欄を見せる（黙って失敗しない）
- [x] **どの欄にも自動では値を入れない**（誤った振り分けを消す手間を作らない）
- [x] すべての欄を手で編集できる
...
- [ ] 詳細画面に全フィールドと名刺画像（表・裏）が表示される
- [ ] 名刺画像をタップすると拡大表示できる（表・裏それぞれ）
...
|---|---|---|
| 読み取った文字の行コピー（コピー内容・表裏の出し分け・失敗時の誘導） | `src/components/OcrTextPanel.test.tsx` | 自動 |
...
**自動テスト 161 / Lint / 型 / ビルド すべて green（CI でも同じ検証ループが通っている）。**
> 件数が 189 から減っているのは、自動振り分けをやめて parser とその 58 件のテストを削除したため。
> 代わりに行コピー・表裏 2 枚・スキーマの表裏読み分けのテストを足している。 上の受け入れ条件のうち `[x]` は検証済み、`[ ]` は**未確認**。
...
| 行コピーと表裏 2 枚 | Testing Library でクリップボードの中身まで assert。コピー失敗時に手動コピー欄が出ることも確認 |
| **実際の OCR（架空の名刺を合成して実行）** | 開発サーバで canvas から架空の名刺画像を作り、表・裏の 2 枚を実際に読み取らせた。表 5 行・裏 3 行を検出し、表裏が分かれて表示されること、2 枚のプレビューが出ることを確認 |
...
| **空白詰め** | 実測で `株 式 会 社 サ ンプ ル` → `株式会社サンプル` に詰まり、`山田 太郎` の空白と `TEL 03-1234-5678` は保持されることを確認 |
| **コピー失敗時のフォールバック** | ブラウザが `NotAllowedError` を返す状況を実際に踏み、案内文・手動選択欄・文字列の全選択・フォーカスまで確認 |
...
- 実機のクリップボードで**コピーが成功する**こと（失敗時のフォールバックは実ブラウザで確認済み。
  headless ではプログラム的なクリックに user activation が無く `NotAllowedError` になるため、成功経路だけ未確認）
...
- [x] ~~**T7 項目抽出 parser**~~ → **取りやめ**。自動振り分けは誤りのコストが大きいためやめ、
      読み取った文字を行ごとにコピーできるパネル（`src/components/OcrTextPanel.tsx`）に置き換えた。
      理由は [architecture.md](architecture.md#項目の振り分けはしない方針転換)
