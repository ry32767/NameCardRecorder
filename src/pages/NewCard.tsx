...
type Phase = 'choosing' | 'reading' | 'ready' | 'saving'
...
export function NewCard({
  ...
  function update<K extends keyof CardFields>(key: K, value: CardFields[K]) {
    ...
    setErrors((current) => {
      ...
      if (key === 'name') delete next.company
      if (key === 'company') delete next.name
  ...
  async function handleFile(file: File | undefined) {
    ...
    try {
      ...
      setFields((current) => ({ ...current, ...parsed, ocrText: result.text }))
      setPhase('ready')
    } catch {
      ...
      setMessage({
        tone: 'info',
        text: '文字を読み取れませんでした。フォームに直接入力して登録できます。',
      })
    }
  }
  ...
  async function handleSave() {
    ...
    if (!repoRef) {
      setMessage({ tone: 'error', text: '設定が未完了です。設定画面でリポジトリとトークンを入力してください' })
      return
    }
    ...
    try {
      const card = await createCard(repoRef, {
        fields,
        ...(image ? { imageBase64: image.storageBase64 } : {}),
      })
      onCreated(card)
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-2xl px-4 py-4 pb-28">
        <ImagePicker image={image} reading={reading} progress={progress} onFile={handleFile} />
        ...
        <section className="mt-6 space-y-4 rounded-card border border-rule bg-card p-4 shadow-card sm:p-6">
          <h2 className="text-title font-bold text-ink">読み取った内容を確認</h2>
          <p className="text-meta text-ink-faint">
            自動で入った内容は候補です。間違っていれば直してから登録してください。
          </p>
